...
È necessario esaminare i diversi metodi di autenticazione supportati per il database Oracle. Per informazioni, vedere link:../install/concept_authentication_methods_for_your_credentials.html["Metodi di autenticazione per le credenziali"^].
...
+
È anche possibile creare una credenziale facendo clic su image:../media/add_icon_configure_database.gif["icona add (aggiungi) nella schermata di configurazione del database"].
